...
if (isset($_GET['op'])) {
        ...
        if ($result) {
            while ($row = mysqli_fetch_assoc($result)) {
                $data[] = [
                    ...
                    'role' => $row['role'], // Menambahkan Role dari database
                    'address' => $row['address'],
                    'city' => $row['city'],
    ...
    if ($op == 'create' && $method == 'POST') {
        // Mengamankan input dari SQL Injection sederhana
        ...
        $role = mysqli_real_escape_string($conn, $input['role'] ?? 'user'); // Role ditambahkan
        ...
        $postalCode = mysqli_real_escape_string($conn, $input['postalCode'] ?? '');
        $password = password_hash("123456", PASSWORD_DEFAULT); // Default password

        $sql = "INSERT INTO users (first_name, last_name, email, phone_number, password, role, address, city, province, postal_code) 
                VALUES ('$firstName', '$lastName', '$email', '$phone', '$password', '$role', '$address', '$city', '$province', '$postalCode')";
    ...
    if ($op == 'update' && $method == 'POST') {
        ...
        $sql = "UPDATE users SET 
                ...
                role='$role', address='$address', city='$city', province='$province', postal_code='$postalCode'
                WHERE id=$id";
...
<head>
    ...
    <title>Manajemen User - Admin Hardjadinata Karya Utama</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .search-box input:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(0, 33, 165, 0.1); }
        ...
        .status.admin { background-color: rgba(0, 33, 165, 0.15); color: var(--primary); border: 1px solid var(--primary);}
        .status.user { background-color: #f1f3f4; color: #5f6368; border: 1px solid #dadce0;}
        ...
        .close-modal { background: none; border: none; color: white; font-size: 24px; cursor: pointer; line-height: 1; transition: all 0.3s;}
        ...
        .form-control, .form-select { width: 100%; padding: 12px 15px; border: 1px solid var(--light-gray); border-radius: var(--border-radius); font-size: 15px; transition: border 0.3s; background-color: white; }
        .form-control:focus, .form-select:focus { outline: none; border-color: var(--primary); box-shadow: 0 0 0 3px rgba(0, 33, 165, 0.1); }
...
<body>
    <aside class="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item"><a href="pelanggan.php" class="nav-link active"><i class="fas fa-users"></i><span>Users</span></a></li>
    ...
    <main class="main-content">
        ...
        <div class="stats-cards">
            ...
            <div class="card fade-in" style="animation-delay: 0.1s;">
                ...
                <div class="card-value">Admin / User</div>
                <div class="card-change">Mengelola Data Internal</div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="customerForm">
                    ...
                    <p style="font-size: 12px; color: var(--danger);"><i>*Password otomatis di-set default: 123456</i></p>
        ...
        <div class="modal-content" style="max-width: 500px;">
            ...
            <div class="modal-body">
                <p style="font-size: 16px; line-height: 1.6;">
                    ...
                    Yakin ingin menghapus user ini dari database? Tindakan ini tidak dapat dibatalkan.
                </p>
            </div>
            <div class="modal-footer">
    ...
    <script>
        ...
        async function saveCustomer() {
            const data = {
                ...
                role: document.getElementById('role').value, // Ambil Value Role
            ...
            if (!data.firstName || !data.lastName || !data.email || !data.phone) {
                showNotification('Nama, Email, dan Telepon wajib diisi!', 'danger');
                return;
            }
        ...
        function renderCustomers() {
            ...
            data.forEach(c => {
                ...
                // Cek Role untuk memberikan style badge yang berbeda
                let roleBadge = '';
                if(c.role === 'admin'){
                    roleBadge = '<span class="status admin"><i class="fas fa-shield-alt" style="margin-right:4px;"></i>Admin</span>';
                } else {
                    roleBadge = '<span class="status user"><i class="fas fa-user" style="margin-right:4px;"></i>User</span>';
                }
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <div class="actions">
                            <button class="action-btn view-btn" onclick="openDetail(${c.id})" title="Lihat"><i class="fas fa-eye"></i></button>
                            <button class="action-btn edit-btn" onclick="openEdit(${c.id})" title="Edit"><i class="fas fa-edit"></i></button>
                            <button class="action-btn delete-btn" onclick="openDelete(${c.id})" title="Hapus"><i class="fas fa-trash"></i></button>
                        </div>
                    </td>
                `;
        ...
        function openEdit(id) {
            ...
            document.getElementById('role').value = c.role || 'user'; // Mengisi nilai dropdown Role
                ...
                <div style="display:flex; gap:15px; align-items:center; margin-bottom:20px;">
                    ...
                    <div>
                        ...
                        <p style="margin-top: 5px;">
                            ID: ${c.id} | Terdaftar: ${formatDate(c.createdAt)} 
                            <span style="display:inline-block; margin-left:10px; padding:2px 8px; border-radius:12px; font-size:12px; background:${c.role === 'admin' ? 'var(--primary)' : '#e0e0e0'}; color:${c.role === 'admin' ? 'white' : '#333'}">${c.role.toUpperCase()}</span>
                        </p>
                    </div>
                </div>
                <div class="form-row" style="margin-bottom:15px; padding: 15px; background: #f8f9fa; border-radius: 6px;">
        ...
        document.getElementById('addCustomerBtn').onclick = () => {
            ...
            document.getElementById('role').value = 'user'; // Set default kembali ke user
            els.modal.style.display = 'flex';
        };
